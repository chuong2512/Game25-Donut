...
#if UNITY_EDITOR
    [CustomEditor(typeof(EasyTextDeformer))]
    public class EasyTextDeformerInspector : Editor
    {
        private EasyTextDeformer curve;
        ...
        {
            if (!curve) curve = target as EasyTextDeformer;
            if (!curve) return;
            ...
            ShowPivot();
            //  curve.DisplayCatmullRomSplineEL(true);
        }

        void OnEnable()
        {
            curve = target as EasyTextDeformer;
