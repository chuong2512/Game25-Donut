...
namespace DonutMatach
{
    public class ProcCurve
    {
        ...
        public static Vector3 [] HelixPoints(Vector3 center, float angleDeg, float k, int pointsCount)
        {
            if (pointsCount == 0 || pointsCount == 1 || angleDeg == 0)
            {
                return new Vector3[] { center, center };
            }

            Vector3[] result = new Vector3[pointsCount];
            float dAngle = angleDeg / (pointsCount - 1);
            float dAngleR = Mathf.Deg2Rad * dAngle;
            float dR = dAngleR * k;
            float currAngle = 0;
            float currR = 0;
            for (int i = 0; i < pointsCount; i++)
            {
                result[i] = new Vector3(Mathf.Cos(currAngle) * currR, Mathf.Sin(currAngle) * currR, 0);
                currAngle += dAngleR;
                currR += dR;
            }
