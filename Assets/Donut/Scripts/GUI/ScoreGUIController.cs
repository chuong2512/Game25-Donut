...
namespace DonutMatach
{
    public class ScoreGUIController : MonoBehaviour
    {
        ...
        [SerializeField] private GameObject ScoreComplete;
        [SerializeField] private Text scoreAmountText;
        [SerializeField] private bool showTargetScore;
        [SerializeField] private string targetScoreSeparator;
        [SerializeField] private bool showOnlyIfTarget = false;
        [SerializeField] private bool showTargetInEditMode = true;
        ...
        {
            get { return GameBoard.Instance; }
        }

        private WinController WController
        {
            get { return MBoard ? MBoard.WinContr : null; }
        }

        private GameConstructor GConstructor
        {
            get { return (MBoard) ? MBoard.gConstructor : null; }
        }
        ...
        private IEnumerator Start()
        {
            ...
            if (GameBoard.GMode == GameMode.Play)
            {
                ...
                if (showScore)
                {
                    ...
                    targetScoreString = (isScoreLevel && showTargetScore)
                        ? targetScoreSeparator + WController.ScoreTarget
                        : "";
                    if (scoreAmountText)
                        tween = new TweenIntValue(scoreAmountText.gameObject, ScoreHolder.Count, 1, 3, true, (s) =>
                        {
                            if (this && scoreAmountText) scoreAmountText.text = GetScoreString(s);
                        });
                }
            }
#if UNITY_EDITOR
        ...
        private void Refresh()
        {
            if (GameBoard.GMode == GameMode.Play)
            {
                ...
                if (ScoreComplete && MBoard.WinContr != null)
                    ScoreComplete.SetActive(
                        WController.HasScoreTarget && (ScoreHolder.Count >= WController.ScoreTarget));
            }
#if UNITY_EDITOR
            else
            {
        ...
        private void ChangeScoreHandler(int score)
        {
            ...
            if (ScoreComplete && WController)
                ScoreComplete.SetActive(WController.HasScoreTarget && (score >= WController.ScoreTarget));
        }
