...
namespace DonutMatach
{
    ...
    {
        [Header("Stars")] [SerializeField] private Image star1;
        [SerializeField] private Image star2;
        [SerializeField] private Image star3;
        [SerializeField] private GameObject starPrefab;


        [Space(8)] [Header("Moves and Time")] [SerializeField]
        ...
        [Space(8)] [Header("Targets")] [SerializeField]
        private GameObject targetPrefab;

        [SerializeField] private RectTransform targetsContainer;

        [Space(8)] [Header("Score strip")] [SerializeField]
        ...
        private void Start()
        {
            ...
            if (GameBoard.GMode == GameMode.Play)
            {
                ...
                {
                    winContr.ChangeMovesEvent += (int m) =>
                    {
                        if (MovesCountText) MovesCountText.text = m.ToString();
                    };
                }
                else
                {
                    winContr.TimerTickEvent += (float t) =>
                    {
                        if (TimerText) TimerText.text = t.ToString();
                    };
                }
            }
        }
        ...
        public void CreateTargets()
        {
            ...
            foreach (var item in MBoard.Targets)
            {
                ...
                item.Value.ChangeCountEvent += (td) =>
                {
                    if (this)
                    {
                        th.SetData(td, true);
                        th.gameObject.SetActive(td.NeedCount > 0);
                    }
                };
        ...
        private void AnimateNewStar(GameObject star, Action completeCallBack)
        {
            ...
            }).AddCompleteCallBack(() => { completeCallBack?.Invoke(); }).SetEase(EaseAnim.EaseOutBounce);
        }
