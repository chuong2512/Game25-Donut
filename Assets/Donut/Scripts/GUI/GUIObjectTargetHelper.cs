...
namespace DonutMatach
{
    public class GUIObjectTargetHelper : MonoBehaviour
    {
        ...
        [SerializeField] private Image unComplete;

        public void SetData(TargetData tData, bool showCount)
        {
            string collectedString = (tData.CurrCount > tData.NeedCount)
                ? tData.NeedCount.ToString()
                : tData.CurrCount.ToString();
            if (countText)
            {
        ...
        public void SetDataInverse(TargetData tData, bool showCount)
        {
            string collectedString = (tData.CurrCount > tData.NeedCount)
                ? "0"
                : (tData.NeedCount - tData.CurrCount).ToString();
            if (countText)
            {
