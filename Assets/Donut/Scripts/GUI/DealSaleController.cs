...
namespace DonutMatach
{
    ...
    {
        [Header("Deal time spans: ", order = 1)] [SerializeField]
        private TimeSpanHolder workingTimeSpan;

        [SerializeField] private TimeSpanHolder pausedTimeSpan;
        ...
        private void StartNewWorkingTimer()
        {
            IsDealTime = true;
            //   SetIAP();
            StartNewTimer(dealWorkingTimerName, workingTimeSpan.GetTimeSpan(), WorkingDealTickRestDaysHourMinSecHandler,
                WorkingDealTimePassedHandler, WorkingDealStartEvent);
        }

        private void StartExistingWorkingTimer()
        {
            IsDealTime = true;
            StartExistingTimer(dealWorkingTimerName, WorkingDealTickRestDaysHourMinSecHandler,
                WorkingDealTimePassedHandler, WorkingDealStartEvent);
        }
        ...
        {
            IsDealTime = false;
            StartNewTimer(dealPausedTimerName, pausedTimeSpan.GetTimeSpan(), PausedDealTickRestDaysHourMinSecHandler,
                PausedDealTimePassedHandler, PausedDealStartEvent);
        }

        private void StartExistingPausedTimer()
        {
            IsDealTime = false;
            StartExistingTimer(dealPausedTimerName, PausedDealTickRestDaysHourMinSecHandler,
                PausedDealTimePassedHandler, PausedDealStartEvent);
        }

        private void StartNewTimer(string timerName, TimeSpan ts,
            Action<int, int, int, float> TickRestDaysHourMinSecAction, Action<double, double> TimePassedAction,
            Action StartAction)
        {
            if (debug) Debug.Log("start new timer: " + timerName);
            gTimer = new GlobalTimer(timerName, ts.Days, ts.Hours, ts.Minutes, ts.Seconds);
            if (TickRestDaysHourMinSecAction != null)
                gTimer.TickRestDaysHourMinSecEvent += TickRestDaysHourMinSecAction;
            if (TimePassedAction != null) gTimer.TimePassedEvent += TimePassedAction;
            StartAction?.Invoke();
            IsWork = true;
        }

        private void StartExistingTimer(string timerName, Action<int, int, int, float> TickRestDaysHourMinSecAction,
            Action<double, double> TimePassedAction, Action StartAction)
        {
